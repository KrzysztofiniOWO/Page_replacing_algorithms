class algorithm_LRU:
    ...
    def replace_pages_LRU(self, pages, frames):
        ...
        for page in pages:
            ...
            if page in operational_frame:
                operational_frame.remove(page)
                operational_frame.insert(0, page)

            else:
                missing_pages = missing_pages + 1

                if len(operational_frame) == frames:
                    operational_frame.pop()
    ...
    def save_to_result_file(self):
        ...
        for frame in available_frames:
            with open(f"LRU_results/{frame}.txt", "w") as f:
                for number in range(1, 51):
                    f.write(f"{self.replace_pages_LRU(self.read_data_from_file(number), frame)}\n")
                f.close()